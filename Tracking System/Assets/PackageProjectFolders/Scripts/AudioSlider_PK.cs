...
public class AudioSlider_PK : Button_PK
{
    ...
    private void Start()
    {
        // Asignar valores iniciales del volumen actual

        if (isMusicSlider)
            slider.value = AudioManager_PK.GetInstance().GetCurrentMusicVolume();
        else
            slider.value = AudioManager_PK.GetInstance().GetCurrentSoundVolume();
